...
public class M_Minimum_LCM
{
    ...
    static long lcm(long a, long b) { long common = gcd((int) a, (int) b); return (a / common) * b; }
    ...
    static long nCr(int n, int r) { if (r > n || r < 0) return 0; long num = factorial(n); long denom = modMul(factorial(r), factorial(n - r)); return modDiv((int) num, (int) denom); } // Added check r < 0
    static int lowerBound(int[] arr, int x) { int low = 0, high = arr.length; while (low < high) { int mid = low + (high - low) / 2; if (arr[mid] < x) low = mid + 1; else high = mid; } return low; } // Avoid potential overflow
    static int upperBound(int[] arr, int x) { int low = 0, high = arr.length; while (low < high) { int mid = low + (high - low) / 2; if (arr[mid] <= x) low = mid + 1; else high = mid; } return low; } // Avoid potential overflow
}
