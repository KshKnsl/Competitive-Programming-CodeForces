...
public class B_Subsequence_Update {
    ...
    public static void main(String[] args) 
    {
        ...
        while (t-- > 0) 
        {
            ...
            long sum=0;
            long min=0;
        long[] leftPart = Arrays.copyOfRange(arr, 0, r);
        long[] rightPart = Arrays.copyOfRange(arr, l, n);
        Arrays.sort(leftPart);
        Arrays.sort(rightPart);

        // Calculate range length and sums
        int rangeLength = Math.min(r - l, Math.min(leftPart.length, rightPart.length));
        long leftSum = 0;
        long rightSum = 0;
        ...
        out.println(Math.min(leftSum, rightSum));
        }
        out.flush();
    }
}