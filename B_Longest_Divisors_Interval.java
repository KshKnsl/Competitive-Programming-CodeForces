...
public class B_Longest_Divisors_Interval {
    ...
    private static long sqrt(long x) { long left = 0, right = Long.MAX_VALUE; while (right > left) { long mid = (left + right) / 2; if (mid * mid > x) right = mid; else left = mid + 1; } return left - 1; }
    private static long pow(long base, long exponent) { long result = 1; while (exponent > 0) { if ((exponent & 1) == 1) result *= base; base *= base; exponent >>= 1; } return result; }
    private static <T extends Comparable<T>> void sort(T[] arr) { List<T> list = new ArrayList<>(Arrays.asList(arr)); Collections.sort(list); for (int i = 0; i < arr.length; i++) arr[i] = list.get(i); }
    private static <T extends Comparable<T>> void rsort(T[] arr) { List<T> list = new ArrayList<>(Arrays.asList(arr)); Collections.sort(list, Collections.reverseOrder()); for (int i = 0; i < arr.length; i++) arr[i] = list.get(i); }
    static long nCr(long n, long r) { long answer = 1, k = min(r, n - r); for (long i = 0; i < k; i++) { answer = (answer % MOD * (n - i) % MOD) % MOD; answer = divide(answer, i + 1); } return answer % MOD; }
    static long divide(long a, long b) { return (a % MOD * (power(b, MOD - 2) % MOD)) % MOD; }
    private static void swap(int[] a, int i, int j) { int t = a[i]; a[i] = a[j]; a[j] = t; }
    private static void swap(long[] a, int i, int j) { long t = a[i]; a[i] = a[j]; a[j] = t; }
    private static void swap(char[] a, int i, int j) { char t = a[i]; a[i] = a[j]; a[j] = t; }
    private static void reverse(int[] a) { int i = 0, j = a.length - 1; while (i < j) swap(a, i++, j--); }
    private static void reverse(long[] a) { int i = 0, j = a.length - 1; while (i < j) swap(a, i++, j--); }
    private static void reverse(char[] a) { int i = 0, j = a.length - 1; while (i < j) swap(a, i++, j--); }
    private static long power(long a,long b) { if (b == 0) return 1L; long ans = power(a, b / 2); ans = modMul(ans, ans); return (b % 2 != 0) ? modMul(ans, a) : ans; }
